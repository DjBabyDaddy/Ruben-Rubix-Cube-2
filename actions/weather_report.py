...
def weather_action(parameters: dict, player=None, session_memory=None):
    ...
    if not city or not isinstance(city, str):
        msg = "Boss, the city is missing for the weather report."
    ...
    try:
        ...
        msg = f"Boss, I couldn't open the browser for the weather report."
        _speak_and_log(msg, player)
        return msg

    msg = f"Showing the weather for {city}, {time}, boss."
    _speak_and_log(msg, player)
...
def _speak_and_log(message: str, player=None):
    ...
    if player:
        try:
            player.write_log(f"RUBE: {message}")
        except Exception:
            pass
