...
# --- DEFERRED LOADING: Vosk model loads in background thread ---
vosk_model = None
rec = None
...
def initialize_vosk():
    """Load the 1.8GB Vosk model in a background thread. Call from main.py at startup."""
    global vosk_model, rec, global_stream
    ...
    # Init audio stream after model is ready
    try:
        device_info = sd.query_devices(kind='input')
        print(f"\nHardware matrix locked onto: {device_info['name']}")
        global_stream = sd.RawInputStream(
...
def record_voice():
    ...
    # Wait for Vosk to finish loading before accepting voice input
    vosk_ready.wait()
